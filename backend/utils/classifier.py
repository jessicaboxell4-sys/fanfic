...
async def _maybe_log_crossover_gap(
    ...
    # 2026-07-07 — Capture the book's source_url on FIRST insert so the
    # suggestion row is self-sufficient: the admin panel can render the
    # ↗ link even if the underlying book is later deleted from the
    # library.  Uses ``$setOnInsert`` so re-classify of the same book
    # doesn't overwrite an already-captured URL (books rarely change
    # their canonical URL, and stable is safer than churny).
    ...
    if source_url:
        on_insert["source_url"] = source_url
