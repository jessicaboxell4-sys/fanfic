...
async def ensure_text_index(db) -> None:
    """Ensure the two Mongo indexes ``book_fulltext`` needs.

    Idempotent — safe to call on every server start.

    * ``text_search_idx`` — a ``$text`` index on ``text`` for the reader's
      full-text search feature.
    * ``book_id_idx`` — a plain b-tree index on ``book_id``.  The admin
      backfill stats endpoint does a ``$in`` query with up to ~7,500
      book_ids against this collection; without an index that query
      hits a 30-second proxy timeout on the live prod dataset (2026-07-07
      incident).  A b-tree turns it from O(N) to O(log N).

    We deliberately keep both indexes single-field so they stay small."""
    existing = await db.book_fulltext.list_indexes().to_list(length=20)
    have = {ix.get("name") for ix in existing}
    if "text_search_idx" not in have:
    ...
    if "book_id_idx" not in have:
        await db.book_fulltext.create_index(
            "book_id",
            name="book_id_idx",
        )
        logger.info("Created book_id_idx on book_fulltext.book_id")
