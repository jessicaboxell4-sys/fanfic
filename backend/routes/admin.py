...
async def list_users(user: User = Depends(require_admin)):
    ...
    rows = await db.users.find(
        ...
        {"_id": 0, "user_id": 1, "email": 1, "name": 1, "is_admin": 1, "is_moderator": 1, "created_at": 1, "last_login_at": 1, "last_seen_at": 1, "last_seen_path": 1,
...
@api_router.get("/admin/presence/online")
async def presence_online(user: User = Depends(require_admin)):  # noqa: ARG001
    """List users active in the last 5 minutes for the Navbar online chip.

    Powers `NavbarOnlineChip` (admin-only pill).  Uses ``last_seen_at``
    stamped by ``auth_dep._touch_last_seen`` on every authenticated
    request.  Excludes test-account fixtures via
    ``mongo_test_account_filter`` so the chip shows only real-user
    activity — matching how the rest of the admin "who's here" surfaces
    filter these out.

    Response shape (consumed by the Navbar chip AND its dropdown menu):
      * ``count``   — int, list length
      * ``users``   — [{user_id, name, email, is_admin, last_seen_at}]

    The window is a hard-coded 5 min because that's what the chip's
    "Online now · last 5 min" label promises the operator.  Sorted
    newest-first so the dropdown surfaces whoever just did something.
    """
    ...
    rows = await db.users.find(
        {
            "$nor": mongo_test_account_filter()["$or"],
            "last_seen_at": {"$gte": window_iso},
        },
        {"_id": 0, "user_id": 1, "name": 1, "email": 1, "is_admin": 1, "last_seen_at": 1},
    ).sort("last_seen_at", -1).limit(50).to_list(length=50)
    # Coerce datetime objects (older Mongo rows) to ISO strings so JSON
    # serialization never blows up.  Newer rows are already ISO strings
    # written by ``_touch_last_seen``.
    for r in rows:
        ts = r.get("last_seen_at")
        if isinstance(ts, datetime):
            r["last_seen_at"] = ts.isoformat()
    return {"count": len(rows), "users": rows}




@api_router.get("/admin/drift-status")
async def drift_status(_user: User = Depends(require_admin)):
    """Latest prod-vs-source drift-check result.

    Powers a small admin console card that surfaces any live-prod
    testids the source is currently missing.  Runs hourly at :17 via
    ``utils.drift_monitor.run_drift_check_once`` — this endpoint just
    returns the most recent stored row.

    Response:
      * ``checked_at``          — ISO timestamp of the last run
      * ``status``              — ``"ok"`` / ``"drift"`` / ``"error"`` / ``"timeout"``
      * ``prod_testid_count``   — how many distinct testids were seen in prod
      * ``drift_count``         — how many are missing from source
      * ``drift``               — list of the actual missing testids
      * ``never_run``           — True iff no result exists yet (freshly
                                  booted pod that hasn't ticked once)
    """
    from utils.drift_monitor import latest_drift_result
    row = await latest_drift_result(db)
    if row is None:
...
async def orphan_audit(
    ...
    Pagination:
        ``limit`` — books to inspect this call (max 2000)
        ``offset`` — how many books to skip before scanning
        Callers should loop until ``scanned < limit`` to cover the
        full library.  Cloudflare's 120s proxy timeout means a single
        big scan (~5000+ books) reliably times out — running in
        smaller windows keeps each call under the ceiling.
    ...
    Returns:
        ...
        total_books  — total books with a filename in the library
    ...
    books_filter = {"filename": {"$exists": True, "$ne": None}}
    total_books = await db.books.count_documents(books_filter)
    ...
    ).sort("book_id", 1).skip(offset).to_list(length=limit)
    ...
    # Parallelise HEAD probes — 64-wide concurrency keeps a 1000-book
    # window under ~15s wall-clock at typical R2 HEAD latency (50-150ms)
    # while staying well under Cloudflare's request-per-worker soft cap.
    # Frontend paginates through the whole library via ``offset`` so
    # each call comfortably clears the 120s proxy timeout.
...
async def admin_list_crossover_suggestions(
    ...
    # characters appear.  Two-phase join:
    #
    #   Phase 1 (fast): exact ``(title, author)`` match against
    #     ``books``.  Uses no index but the $or clauses are small.
    #   Phase 2 (fallback): for any suggestion still without a URL,
    #     do a case-insensitive regex lookup — catches "Dead To Love"
    #     vs "Dead to Love" post-rename drift (2026-07-07 report).
    #
    # We also PERSIST any matched URL back onto the suggestion row so
    # a later book-deletion doesn't strip it away.  After a suggestion
    # has been enriched once, its ``source_url`` survives on the
    # ``crossover_suggestions`` document forever.  ``book_present`` on
    # each returned row tells the frontend whether the underlying book
    # still exists in the library (renders a "book removed" hint when
    # False).
    if rows:
        import re as _re
        ...
        # book_by_pair_exact keeps the whole doc so we can distinguish
        # "book exists but has no source_url" vs "no book at all".
        book_by_pair: Dict[tuple, dict] = {}
        if or_clauses:
            cursor = db.books.find(
                {"$or": or_clauses},
            ...
            async for b in cursor:
                ...
                book_by_pair.setdefault(key, b)

        # Phase 2 — case-insensitive fallback for rows that didn't
        # match exactly.  Small N (typically ≤ page size) so an
        # individual find_one per unmatched row is fine — stops on
        # first hit.
        for r in rows:
            key_exact = (str(r.get("title") or ""), str(r.get("author") or ""))
            if key_exact in book_by_pair:
                continue  # already have exact match
            t = (r.get("title") or "").strip()
            a = (r.get("author") or "").strip()
            if not (t and a):
                continue
            b_ci = await db.books.find_one(
                {
                    "title":  {"$regex": f"^{_re.escape(t)}$", "$options": "i"},
                    "author": {"$regex": f"^{_re.escape(a)}$", "$options": "i"},
                },
                {"_id": 0, "title": 1, "author": 1, "source_url": 1},
            )
            if b_ci:
                book_by_pair[key_exact] = b_ci

        # Attach + opportunistic write-back
        for r in rows:
            key = (str(r.get("title") or ""), str(r.get("author") or ""))
            matched = book_by_pair.get(key)
            # `book_present` reflects whether the underlying book is
            # STILL in the library.  A prior `source_url` persisted on
            # the suggestion row doesn't count — that's a snapshot, the
            # book may have been deleted since.
            r["book_present"] = bool(matched)
            if matched and matched.get("source_url") and not r.get("source_url"):
                r["source_url"] = matched["source_url"]
                # Persist so future page loads don't need the join,
                # AND so a later book-delete can't strip the URL.
                try:
                    await db.crossover_suggestions.update_one(
                        {"dedup_key": r.get("dedup_key")},
                        {"$set": {"source_url": matched["source_url"]}},
                    )
                except Exception:
...
# ---------------------------------------------------------------------
# POST /api/admin/purge-test-account-spam
# ---------------------------------------------------------------------
# One-shot cleanup for the "test account friend-request spam" incident
# (2026-07-09).  Multiple `Iter{n} Tester` seed users had accumulated
# pending friend requests + notifications on Jessica's live prod inbox
# before we shipped the block in ``routes/friends.py``.  This endpoint
# lets an admin drain the residual data whenever they want — safe to
# re-run (idempotent: only deletes rows whose requester or subject is
# a test-account user).
#
# Uses the shared ``mongo_test_account_filter`` so future changes to
# what counts as a "test account" propagate automatically.

@api_router.post("/admin/purge-test-account-spam")
async def purge_test_account_spam(user: User = Depends(require_admin)):
    # 1. Enumerate test-account user_ids + display names in one pass.
    test_users_cur = db.users.find(
        {**mongo_test_account_filter()},
        {"_id": 0, "user_id": 1, "name": 1, "username": 1, "email": 1},
    )
    test_user_ids: list[str] = []
    test_display_names: list[str] = []
    async for u in test_users_cur:
        if u.get("user_id"):
            test_user_ids.append(u["user_id"])
        disp = (u.get("name") or u.get("username") or "").strip()
        if disp:
            test_display_names.append(disp)

    # 2. Drop pending friendship rows where the requester is a test
    #    account.  Do NOT touch accepted friendships — those may have
    #    been auto-accepted before this cleanup existed.
    friendships_deleted = 0
    if test_user_ids:
        res = await db.friendships.delete_many({
            "status": "pending",
            "requested_by": {"$in": test_user_ids},
        })
        friendships_deleted = int(getattr(res, "deleted_count", 0) or 0)

    # 3. Drop friend_request notifications whose title starts with any
    #    known test-account display name.  We match on title because
    #    the ``create_notification`` payload doesn't carry a sender
    #    user_id — the requester is embedded in the title string
    #    ("<Name> wants to be friends").  Case-insensitive.
    notifs_deleted = 0
    if test_display_names:
        import re as _re
        or_clauses = [
            {"title": {"$regex": f"^{_re.escape(name)} wants to be friends", "$options": "i"}}
            for name in test_display_names
        ]
        res_n = await db.notifications.delete_many({
            "kind": "friend_request",
            "$or": or_clauses,
        })
        notifs_deleted = int(getattr(res_n, "deleted_count", 0) or 0)

    await record_admin_action(
        user,
        "friends.purge_test_account_spam",
        metadata={
