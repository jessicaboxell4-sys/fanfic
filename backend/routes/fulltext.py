...
async def _run_backfill_batch(limit: int) -> None:
    ...
    Per-batch counters (``batch_scanned``, ``batch_indexed``,
    ``batch_missing_file``, ``batch_errors``) are reset at the top of
    every invocation.  With the auto-continue mode in the UI kicking
    successive batches back-to-back, resetting here means the
    "This run: N of 500" sub-line always reflects THIS batch, not a
    ...
    try:
        ...
        # Skip previously-flagged orphans — their storage backing was
        # confirmed missing on a past walk, so they've been "taken out
        # of the equation" and won't show up as work-to-do.
        ...
        for b in candidates:
            ...
            if not ok:
                ...
                # Flag the book row as orphaned so future walks skip it
                # AND ``/admin/fulltext/stats`` doesn't keep counting it
                # toward "total remaining to index".  Note: we DON'T
                # delete the book here — the operator can still see and
                # bulk-purge these via `/admin/orphan-audit` (the
                # HEAD-probe audit endpoint in routes/admin.py).
                await db.books.update_one(
                    {"book_id": book_id},
                    {"$set": {"storage_missing_at": datetime.now(timezone.utc).isoformat()}},
                )
...
async def fulltext_stats(_user: User = Depends(require_admin)):
    ...
    Fields:
      * ``total_active`` — books not in Trash and not orphaned (matches
        what the walker will actually try to index).  Books flagged
        ``storage_missing_at`` are excluded — they've been taken out of
        the equation.
      * ``indexed`` — rows in ``book_fulltext`` that still map to an
        active, non-orphaned book.  Survives pod restarts (comes
        straight from Mongo).
      * ``remaining`` — ``total_active - indexed``, clamped at 0.
      * ``pct`` — 100 when total is 0, else rounded ratio.
      * ``orphaned`` — books flagged as storage-missing by the walker
        (informational — surfaced as a pill in the admin UI with a
        deep-link to the orphan-audit card).
    ...
    """
    total_active = await db.books.count_documents(
        {"category": {"$ne": "Trash"}, "storage_missing_at": {"$exists": False}}
    )
    active_ids = {
    ...
    orphaned = await db.books.count_documents(
        {"storage_missing_at": {"$exists": True}}
    )
    return {
