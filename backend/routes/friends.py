...
async def send_friend_request(body: FriendRequestBody, user: User = Depends(get_current_user)):
    # 2026-07-09 — Block test-fixture accounts from ever initiating a
    # friend request.  The seeded ``Iter{n} Tester`` accounts were
    # spamming the real user's notifications on the preview + prod
    # environments (see screenshot 2026-07-09).  Real users signing up
    # from an @-domain not on the test list are unaffected.  See
    # ``utils/test_account_filter.is_test_account`` for the rule set.
    if is_test_account(getattr(user, "email", None)):
        logger.info(
            "friend_request.blocked_from_test_account requester=%s email=%s",
            user.user_id, getattr(user, "email", None),
        )
        raise HTTPException(
            status_code=403,
            detail="Test accounts can't send friend requests.",
        )
    target = None
