...
const FALLBACK_WHATS_NEW = {
  ...
  items: [
    ...
    { to: "/account/safety", label: "One-click virus scan after upload", desc: "— uploads are fast because the AV scan happens just after, not during. A sticky toast pops up after each batch with a Scan now button; one tap sweeps your whole library." },
        ...
        <div className="grid md:grid-cols-1 gap-10">
          <article className="min-w-0 break-words">
            ...
            <Section id="antivirus" icon={Shield} title="Antivirus &amp; library safety">
              <p>Shelfsort runs <strong>ClamAV</strong> over your library to catch malicious files. To keep uploads fast, the scan happens <em>just after</em> the file lands — not during the upload itself — so big batches don&apos;t wait on the scanner.</p>
              <ul>
                <li><strong>The flow</strong> — when you finish uploading, a sticky toast pops up: <em>&ldquo;X new books ready · scan for viruses?&rdquo;</em> with a one-click <em>Scan now</em> button. Tap it, and ClamAV sweeps your whole library (newly-added books plus any older ones) and reports back. Books arrive marked as <em>&ldquo;unscanned&rdquo;</em> until that sweep completes.</li>
                <li><strong>What gets scanned</strong> — every book in your library when you trigger a scan, every backup-ZIP restore, every photo attached to feedback, and every file restored back from cloud storage on download. The signature database is refreshed daily, so files that were clean yesterday can still be caught if a brand-new threat is published.</li>
              ...
              <p className="text-xs text-[#6B705C]">If the AV scanner is temporarily unavailable (signatures still downloading, daemon restarting), uploads still complete and the books stay flagged as <em>unscanned</em> until the daemon is back — your next &ldquo;Scan now&rdquo; tap will sweep them.</p>
            </Section>
