...
export default function Help() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF6EE]">
      <SkipToContent />
      <Navbar />
      <main id="main-content" tabIndex={-1} className="px-4 lg:px-8 py-10">
        ...
        <div className="grid md:grid-cols-1 gap-10">
          <article className="min-w-0 break-words">
            ...
            <Section id="uploads" icon={Upload} title="Uploading books">
              ...
              <p><strong>Fast and resilient.</strong> Uploads run in parallel through a sliding window of 3 to 6 files (slow-start ramp — begins with 3, climbs to 6 after successive successes, throttles back down when the origin looks strained), recover from individual file failures, and the asynchronous pipeline means you can drop dozens of files at once without worrying about edge-server timeouts. Submit returns in seconds; processing keeps running in the background, and your library refreshes automatically when each batch completes.</p>
              <p><strong>See exactly what&rsquo;s happening (new — 2026-08-27).</strong> Every file gets its own row with an inline progress bar showing which state it&rsquo;s in — <em>Waiting</em>, <em>Uploading</em> (real byte-level percentage), <em>Processing</em> (indeterminet shimmer while the server does AV scan + Claude classification + R2 mirror), <em>Done</em>, <em>Skipped</em> (duplicate), or <em>Failed</em> (with a specific reason and one-click Retry). The list is grouped by status with collapsible headers, has a filename search + status filter for big batches, an ETA chip in the header that starts showing once we have enough data to be honest about the estimate, and a <em>Copy stuck rows</em> button that dumps failed filenames + reasons to your clipboard as Markdown — handy for bug reports. A big <em>Clear all</em> is always available. When a batch finishes 100% clean you get a <em>✨ All N books saved</em> pill; there&rsquo;s an optional gentle bell that plays with it (opt-in via the tiny speaker toggle in the tray, defaults off).</p>
              <p><strong>&ldquo;Stage before upload&rdquo; — review before you commit (default ON).</strong> New drops park in a staging tray so you can see what you selected, remove individual files with the ✕ button, and click <em>Start uploading N</em> when you&rsquo;re ready. Drop more files later to append to the queue; the tray persists across page navigations. Turning it OFF (via the toggle above the drop zone) makes drops auto-upload immediately — same progress list either way. The toggle now stays in sync across the Dashboard, All-books, and every other place you can upload — no more &ldquo;one page says on, the other says off&rdquo; confusion.</p>
              <p><strong>Refresh-safe: auto-resume on re-drop.</strong> If you accidentally close the tab or reload while uploads are in flight, don&rsquo;t worry — the browser can&rsquo;t remember file contents across a refresh, but Shelfsort remembers everything else. Interrupted rows show up as <em>&ldquo;Drop again to resume&rdquo;</em>. Drag the same files back in and Shelfsort auto-matches by filename+size, replaces the interrupted rows with fresh queued rows, and a green toast confirms <em>&ldquo;Auto-resumed N interrupted files from your previous session.&rdquo;</em> A subtle amber banner also appears at the top while any uploads are still running, warning you before you accidentally reload.</p>
              <p><strong>Reliability fix (2026-08-27).</strong> Fixed a long-standing bug in the concurrency loop where roughly 3 files per large batch would silently vanish (never sent to the server, never marked failed, just stuck in <em>Waiting</em> forever). Root cause: the sliding-window concurrency counter was being read at the wrong moment during a slow-start ramp, causing the loop to skip past files. Now snapshots the value at each round start. If you&rsquo;ve been finding you had to re-drop a handful of files after every big upload — that&rsquo;s over.</p>
              <p><strong>Airdrop mode for big drops (new).</strong> Drop more than 20 files at once and Shelfsort kicks into airdrop mode — the bytes land on the server in seconds, and the upload bar hits 100% the moment everything&rsquo;s safely staged. Metadata extraction, Calibre conversion (for PDFs, Kindle, Word), AI classification, cover saving and R2 mirror all happen <em>after</em> the upload completes. You can close the tab the moment the bar finishes; the server keeps grinding through the queue, and a 5-minute backend cron re-kicks anything that gets stuck (backend restart, deploy mid-upload, etc.). Books appear on the library page as each one finishes processing.</p>
              <p><strong>Deferred AI sorting.</strong> The Claude classifier no longer blocks the upload progress bar. Books land in the library immediately — title, author, cover, AO3 tags, source URLs, duplicate detection all run at upload time — and the AI-driven <em>category + fandom</em> step happens in a background polish queue.</p>
              <ul>
                <li>A &ldquo;<strong>✨ N books sorting in the background</strong>&rdquo; banner appears at the top of <Link to="/library/all">All books</Link> while polish is running. You can close the tab — the work continues on the server, and a 5-minute recovery cron sweeps anything that gets stuck (backend restart, missed schedule).</li>
                <li>Each pending card shows a solid <strong>✨ Sort now</strong> chip — one click runs the classifier against that specific book and flips the badge instantly.</li>
                <li>Already-classified cards reveal a faint <strong>✨ Re-sort</strong> chip on hover — useful when Claude picked the wrong fandom. Same endpoint, same effect, no need to open the detail page.</li>
                <li><strong>Hover the category badge</strong> on any card to see <em>why</em> the AI (or heuristic) made that call — e.g. <em>&ldquo;Matched 3 keywords for Harry Potter (e.g. &lsquo;hogwarts&rsquo;) in title/description/sample text.&rdquo;</em></li>
              </ul>
              <p><strong>Batch size — Shelfsort processes 200 books at a time.</strong> Drop 1,000 EPUBs at once and we&rsquo;ll show a friendly prompt (&ldquo;Whoa, that&rsquo;s a big library!&rdquo;), then auto-queue them in 5 sequential batches of 200 with a live <em>Batch 3 of 5</em> progress line — no manual re-dropping. The 200-per-batch ceiling is the only one we enforce, purely to keep the antivirus scanner responsive for everyone. You can wander off; we&rsquo;ll keep going in the background.</p>
              <p><strong>AV-flagged files are never shared.</strong> If a book in your library is marked as infected by the antivirus scan, it&rsquo;s automatically hidden from friends&rsquo; library browsers and any attempt to request it via DM is rejected at the server. Cleanup happens on your end via <Link to="/account/safety">Account safety</Link>.</p>
            ...
            <Section id="discovery" icon={UserIcon} title="Browsing & discovery">
              ...
              <ul>
                ...
                <li><strong><Link to="/library/pairings">Pairings browser</Link></strong> — every ship/relationship across your library with counts and sample titles. Click a pairing to see the books featuring it. Pairings are extracted from EPUB metadata at upload time and canonicalized (alphabetical order, slash delimiter) so identical ships from different sources group correctly. Sorted by book count DESC, with an alphabetical tiebreaker so ties don&apos;t shuffle as you upload.</li>
                <li><strong><Link to="/library/characters">Characters browser</Link></strong> — every character mentioned in your library&apos;s pairings, sorted by book count (alphabetical tiebreak). Click a character to see every book they appear in. Characters are derived at read-time by splitting each book&apos;s relationships on AO3&apos;s <code>/</code> and <code>&amp;</code> separators, so the list updates instantly as you upload new books — no re-parse required.</li>
            ...
            <Section id="antivirus" icon={Shield} title="Antivirus &amp; library safety">
              <p>Shelfsort runs <strong>ClamAV</strong> over your library to catch malicious files. To keep uploads fast, the scan happens later — not during the upload itself — so big batches don&apos;t wait on the scanner.</p>
              <ul>
                <li><strong>The flow</strong> — books arrive marked <em>&ldquo;unscanned&rdquo;</em>. When you next visit <Link to="/library/polish">Polish my library</Link> and apply suggestions, Shelfsort automatically runs ClamAV in the background. The polish page shows a live <em>&ldquo;X / Y books scanned&rdquo;</em> counter with a progress bar so you can watch it sweep through. <strong>Each sweep covers up to 500 books</strong>, picking the ones that need it most first (unscanned books, then oldest-scanned). For libraries larger than 500 books just polish again later — each polish picks the next batch automatically until everything has been covered. You can also run a scan manually from <Link to="/account/safety">Account → Library safety report</Link>.</li>
                <li><strong>What gets scanned</strong> — every book in your library when a scan runs, every backup-ZIP restore, every photo attached to feedback, and every file restored back from cloud storage on download. The signature database is refreshed daily, so files that were clean yesterday can still be caught if a brand-new threat is published.</li>
                <li><strong>Where to check your own report</strong> — <Link to="/account/safety">Account → Library safety report</Link>. Four live counters: clean, flagged, unscanned, total. If anything has been flagged it&apos;s listed by filename + signature, with a timestamp. The <em>Unscanned</em> count tells you how many books are waiting for their first scan.</li>
