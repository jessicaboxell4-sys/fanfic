        ...
        <div>
          ...
          <Section id="orphan-audit" icon={AlertOctagon} title="Orphan audit & cleanup">
            ...
            <ul>
              ...
              <li><strong>Every delete auto-notifies affected users (2026-07-11)</strong> — after each bulk delete, the endpoint writes a permanent per-book record to <code>deleted_books_ledger</code> (never truncated) and creates ONE in-app notification per affected user summarising what they lost, with up to 3 sample titles. Users can never be silently trimmed anymore — every delete here is visible to the people affected.</li>
              <li><strong>Retroactive notice (one-shot, idempotent)</strong> — the <em>&quot;Send retroactive notice&quot;</em> link next to Run audit fires a single-shot notification to every existing user (created BEFORE the last orphan-delete admin action) explaining the maintenance event. New signups joining after the delete are NOT notified. Guarded by a marker in <code>system_flags</code> so double-clicking is a no-op.</li>
