...
function SafetyStat({ icon: Icon, label, value, total, color, testid }) {
  // Palette expressed with data-* attribute + a small CSS mapping in
  // index.css so dark-mode overrides can hook in.  Inline `style`
  // props are avoided here because they have higher specificity than
  // any class-based dark-mode rule and would defeat the theme flip.
  const pct = total > 0 ? Math.round((value / total) * 100) : 0;
  return (
    <div
      className="safety-stat-card rounded-xl border p-4"
      data-safety-color={color}
      data-testid={testid}
    >
      <div className="flex items-center gap-2 mb-2">
        <Icon className="safety-stat-icon w-4 h-4" />
        <p className="safety-stat-label text-xs uppercase tracking-[0.15em] font-bold">
          {label}
        </p>
      </div>
      <p className="safety-stat-value font-serif text-5xl font-bold leading-none">
        {value}
      </p>
      <p className="safety-stat-muted text-xs mt-2 font-medium">
