...
const TOUR_STEPS = [
  {
    ...
    // 2026-07-09 — Was `/dashboard` which does not exist as a route
    // (see App.js — the library IS the dashboard).  Fresh users doing
    // the tour were silently bounced to `/` (the marketing landing)
    // between the welcome and library steps.  Route to `/library`
    // so the tour flow stays inside the app.  Bug found by testing
    // agent iteration 78.
    path: "/library",
  },
  {
