...
import NavbarOnlineChip from "./NavbarOnlineChip";
...
// trigger button.
function NavDropdown({ label, icon: Icon, items, sections, testid }) {
  ...
  // Support two shapes: `sections=[{heading, items}...]` renders
  // sub-groups with a small caps heading between each block; the
  // legacy `items=[...]` prop keeps a flat list.  Callers pick one.
  const renderItem = (it) => (
    <Link
      key={it.to}
      to={it.to}
    ...
    <div ref={wrapRef} className="relative">
      ...
      {open && (
        ...
        >
          {sections ? (
            sections.map((sec, i) => (
              <div key={sec.heading} className={i > 0 ? "mt-1 pt-1" : ""}>
                <p
                  className="px-3 pt-1 pb-0.5 text-[10px] font-semibold uppercase tracking-[0.15em] text-[#5B5F4D]"
                  data-testid={`${testid}-subheading-${sec.heading.toLowerCase().replace(/\s+/g, "-")}`}
                >
                  {sec.heading}
                </p>
...
function SecondaryLinks({ user, unknownFandomCount, crossoverPendingCount, onNavigate, inDrawer = false }) {
  ...
  // Library items split into two sub-groups so the desktop dropdown
  // AND the mobile drawer can both surface the hierarchy the user
  // asked for on 2026-07-09 — "Your books" (physical shelf entries)
  // vs "Insights" (derivative views over that shelf).  Duplicates
  // sits under "Your books" because it's a file-level bucket, Trash
  // under "Insights" because it's a state view.
  ...
  // Flattened list — some legacy call sites still expect the old
  // array shape; keep it as a derived value so we don't diverge.
  const libraryItems = librarySections.flatMap((sec) => sec.items);
  const exportItems = [
    ...
    return (
      <div className="flex flex-col gap-1 w-full">
        ...
        {librarySections.map((section) => (
          <div key={section.heading} className="flex flex-col gap-1 mt-1">
            <p
              className="px-3 pb-0.5 text-[10px] font-semibold uppercase tracking-[0.15em] text-[#5B5F4D]"
              data-testid={`drawer-library-subheading-${section.heading.toLowerCase().replace(/\s+/g, "-")}`}
            >
              {section.heading}
            </p>
  ...
  return (
    <div className="hidden lg:flex items-center gap-2">
      <NavDropdown label="Library" icon={Library} sections={librarySections} testid="navbar-library-menu" />
      ...
      {user.is_admin && (
        <div
          data-testid="navbar-admin-cluster"
          className="inline-flex items-stretch bg-[var(--surface)] border border-[var(--border)] rounded-[0.625rem] overflow-hidden divide-x divide-[var(--border)]"
        >
          <NavbarOnlineChip isAdmin merged />
          <AdminShortcutButton
              ...
              <div className="relative lg:hidden" ref={menuRef}>
                ...
                {menuOpen && (
                  <div
                    ...
                    /* 2026-07-09 — drawer content had grown taller than the
                       viewport on mobile (Library sub-groups + Community +
                       Help + Suggestions all stacked).  Cap the height at
                       just under the viewport and let the interior scroll,
                       so users can reach EXPORT / logout without the drawer
                       spilling off-screen.  `overscroll-contain` stops
                       scroll-chaining into the page beneath. */
                    className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-lg border border-[#E8E6E1] p-2 z-50 max-h-[calc(100vh-5rem)] overflow-y-auto overscroll-contain"
                  >
                    <SecondaryLinks
                      user={user}
                      unknownFandomCount={unknownFandomCount}
