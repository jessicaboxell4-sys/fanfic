...
export default function BookQuickSearch() {
  ...
  return (
    <div ref={wrapRef} className="relative w-full" data-testid="book-quick-search">
      ...
      />
      {loading && <Loader2 className="absolute right-2 top-1/2 -translate-y-1/2 w-3 h-3 text-[#6B705C] animate-spin" />}
          ...
          {results.map((b) => (
            <li key={b.book_id}>
              ...
              >
                ...
                {b.author && <p className="text-[10px] text-[#6B705C] truncate">by {b.author}</p>}
                {b.category && (
      ...
      {open && q.trim().length >= 2 && !loading && results.length === 0 && (
        <div
          ...
          className="absolute left-0 right-0 top-[36px] z-30 bg-white border border-[#E5DDC5] rounded-lg shadow-lg px-3 py-2 text-xs text-[#6B705C]"
        >
