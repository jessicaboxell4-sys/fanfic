...
| 20 | Pairing intersection in Pick-for-me weighting | ~5 min | Low-Med | The iter-63 Pick-for-me weighting (`AllBooksPage.jsx` chip-pick-for-me handler) currently scores books by overlap with finished books' fandom (×3), category (×2), and author (×2). Add `+ 2 × (pairing intersection size)` since readers stick with their ships harder than their fandoms — especially for fic-heavy libraries. The `book.pairings` array is already denormalized on every book row, so this is a one-line addition to the weight formula. Tiny, but for fic-heavy users it should make Pick-for-me feel noticeably more on-target. Reminder added 2026-06-27. |
