...
| 11 | Trending books strip on landing | ✅ DONE 2026-06-29 (verified) — `TrendingBooksStrip.jsx` (114 LOC) already imported + rendered in `Landing.jsx` L347 directly under `FeaturedReadersStrip`. Pulls `/api/books/trending?limit=8&days=7` (anon-accessible). Silent-when-empty fallback so the landing never shows an empty state. Each row deep-links to a sample owner's public library. Roadmap entry was stale — the work landed in an earlier session but wasn't marked done. |
...
| 21 | Classifier reliability dashboard in AdminConsole | ✅ DONE 2026-06-29 — `GET /api/admin/classifier/reliability?days=7` aggregates polish-failed rows: 3-tile totals (recent / currently stuck / permanently stuck ≥3 attempts), top-5 error fingerprints (lower-cased first-80-char group), by-attempt distribution. New `ClassifierReliabilityCard` in AdminConsole between Stuck Uploads and Health. Refreshes 60s when tab visible. Tested with seeded data: ranking + grouping + permanently-stuck flag all correct. |
| 22 | Pair-aware text-contrast lint v2 | ~1 h | Med | v1 of `scripts/check_text_contrast.py` (shipped 2026-06-29) checks every `text-[#…]` against the paper bg `#FDFBF7`. It MISSES pill/badge cases like `bg-[#FBE2E0] text-[#7C2D2A]` where the actual perceived contrast depends on the colored bg in context. v2 walks the JSX upward (or matches sibling `className` tokens on the same element) to find the enclosing `bg-[#…]` and compute the foreground/background pair-specific WCAG ratio. Tricky edge: dynamic backgrounds (variables, ternaries) — for those, skip-with-warning. Reminder added 2026-06-29. |
| 23 | "Retry from server" bulk button on failed-uploads banner (Phase 2) | ~60 min including testing | Med | **2026-06-29 scope re-estimate**: bigger than first thought. `_run_upload_job`'s `finally` block unconditionally sweeps the staging dir on completion (success OR failure), so `bytes_available=true` is never set anywhere today — the Phase-1 component's button placeholder would always show 0 even if the endpoint shipped. Proper implementation needs **four** steps: (1) move staging dirs to `_retry_staging/<failure_id>/` quarantine BEFORE the finally sweep on recoverable failure stages (av, classify, extract — NOT network/submit where bytes never arrived); (2) new `POST /api/uploads/failures/retry-server` that re-spins each quarantined dir into a fresh upload_jobs row + kicks `_run_upload_job`; (3) stale-quarantine cleanup cron (7-day sweep so abandoned retries don't fill disk); (4) frontend "Retry on server (N)" button + tooltip. Touches the upload hot path — requires testing-agent regression run. |
