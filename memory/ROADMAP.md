...
| 15 | Profile-completeness meter + extensions | ~20 min base, more if extended | Med-High | The `/auth/me` payload now carries `username`, `bio`, `library_visible_to_public` (iter 56). Compute a 0-3 score and surface it as: (a) 3-dot meter on `/account`; (b) first-login wizard; (c) "Almost there!" toast on each save; (d) default sort on `/users` directory by score so polished profiles bubble up; (e) Featured Readers eligibility floor (score ≥ 2/3); (f) ✨ stamp at 3/3; (g) re-engagement email ladder; (h) admin histogram. Top pick: directory sort (single Mongo sort key, day-1 impact). Reminder added 2026-06-27. |
