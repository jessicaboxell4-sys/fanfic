...
| 21 | Classifier reliability dashboard in AdminConsole | ~30 min | Med | Now that polish_attempts + polish_last_error + polish_failed_at are tracked per book (iter 57 polish retry inbox), surface aggregate health in AdminConsole: top error fingerprints last 24h (group by first 80 chars of polish_last_error), retry-success-rate per attempt # (1st vs 2nd vs 3rd), books-permanently-stuck count over time (currently in polish-failed with polish_attempts ≥ 3). New endpoint `GET /api/admin/classifier/reliability?days=7`. Helps spot patterns ("12% of failures = 'invalid JSON from Claude'") and tune the classifier prompt before users feel it. Reminder added 2026-06-28. |
...
| 23 | "Retry from server" bulk button on failed-uploads banner (Phase 2) | ~40 min | Med | `FailedUploadsList.jsx` already has the comment "bytes_available === true → reserved for Phase 2. We would call POST /uploads/failures/:id/retry to re-run the pipeline server-side without re-uploading." Wire it: backend endpoint `POST /api/uploads/failures/retry-server` that finds all failure rows where `bytes_available=true` for the user and re-enqueues the upload_jobs row + clears the failure. Frontend: third button "Retry on server (N)" only enabled when ≥1 row has bytes_available. Saves user the re-upload bandwidth when the original bytes are still on R2 staging. Reminder added 2026-06-29. |
