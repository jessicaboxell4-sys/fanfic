...
missed a whole class of regressions where a testid is preserved but the
surrounding user-facing copy is stripped. Added a companion sentinel that
grep-checks a curated set of load-bearing UI strings against the shipped
prod bundle.

### Shipped

- **NEW `/app/scripts/deploy_text_sentinel.py`** — 24 curated sentinels
  spanning Users & admins, Drift status card, Feedback inbox, sign-up gates,
  Quarantine page, Duplicate dismissals, Navbar, upload / library core,
  reader, and admin attribution timeline.
- Handles the JS-bundle `\uXXXX` unicode-escape gotcha — non-ASCII chars
  like the `↔` in "Prod ↔ source drift" are checked against BOTH the raw
  UTF-8 form and the escaped form (webpack/Terser encodes non-ASCII
  literals as `\uXXXX` in the minified output).
- CLI mirrors `deploy_drift_check.py`: fetches the live prod bundle by
  default, `--bundle F` for a cached file, `--built` for a local
  `/app/frontend/build/static/js/main.*.js`.
- Exit code 1 on any missing sentinel — safe to gate deploys behind if we
  want to make it a hard block later.

### Sentinel scan of current prod (`main.ac1fb135.js`)

- **22/24 sentinels present** — no other regressions detected. The two
  "misses" are `Online:` and `Offline:` (the presence-pill fix I applied
  earlier this session — will clear on next deploy).
- Notable false positive I removed after first-pass tuning: `Wake Lock` /
  `Keep me awake`. Those strings only exist in *comments* in
  `UploadZone.jsx`; the Wake Lock feature is UI-silent (invokes the
  browser API, no visible toggle). Comments get stripped by Terser.

### Adding new sentinels

Rules of thumb (documented in the script docstring):
1. Pick strings that are unique on the page (avoid "OK", "Save").
2. Pick strings whose absence would visibly hurt UX (card titles,
   empty-state headlines, primary action buttons).
3. Encode as they render (JSX entities like `&apos;` are already decoded
   to `'` in the built bundle — use the decoded form).
4. Group by surface so failure messages are actionable.

---
## 2026-07-09 (post-deploy) — Users & admins presence-pill visual regression fix

Deploy went out successfully (`main.f3b98cff.js` → `main.ac1fb135.js`) but user
reported the Users & admins card had lost its presence-pill design: the header
no longer showed `Online: N · Offline: N` counts and per-row presence collapsed
to a tiny colored dot with no text. Drift check hadn't flagged it because it
only tracks testids, not text/layout.

### Shipped

- **`frontend/src/pages/AdminConsole.jsx`** — `UsersCard`:
  - Extracted `isUserOnline(u)` and `fmtActive(iso)` helpers (5-min window,
    "just now" / "Xm ago").
  - "Real users" header now renders two count chips beside `(N)`:
    `admin-users-online-count` (green dot + `Online: N`) and
    `admin-users-offline-count` (grey dot + `Offline: N`).
  - Per-row presence indicator upgraded from a bare dot to a full pill badge
    (`admin-user-presence-<id>`) with:
    - green mint bg + `Online` label + `· active <relative>` sub-label when
      last_seen_at is within 5 min
    - beige bg + `Offline` label otherwise
  - Preserved existing `data-online` attribute and the hover-title text.

### Testids added

`admin-users-online-count`, `admin-users-offline-count`.

### Testids preserved (regression check)

`admin-users-card`, `admin-users-list`, `admin-user-row-<id>`,
`admin-user-presence-<id>`, `admin-user-toggle-<id>`,
`admin-user-mod-toggle-<id>`, `admin-user-last-login-<id>`,
`admin-users-test-collapse`, `admin-users-test-list`.

### Verified

Testing agent v3 (iteration 61) — 100% frontend pass. Header chips render
`Online: 0` / `Offline: 28`; all 28 real-user pills carry the `OFFLINE` label
with `data-online="false"`; test-user collapse still expands and its rows
also carry the pill. Only caveat: preview DB had no online users at test
time, so the green online branch was code-verified but not visually asserted.

---
## 2026-07-09 (later) — Admin drift-status card

Turned the invisible `GET /api/admin/drift-status` endpoint into a
first-class Admin Console card so future prod-vs-source drift is
surfaced at a glance instead of requiring manual
`python3 /app/scripts/deploy_drift_check.py` runs before every deploy.

### Shipped

- **`frontend/src/pages/AdminConsole.jsx`** — new
  `DriftStatusCard` component + registry entry (System & health,
  testid `admin-drift-status-card`).
- Renders a colour-coded status badge with `data-status` attribute:
  * `ok` — green "Safe to deploy" (fresh 0-drift check)
  * `drift` — red "N testids would regress" + expandable list of the
    actual missing testids (`admin-drift-status-list`, one
    `admin-drift-item-{testid}` per row)
  * `timeout` / `error` — amber warning with the underlying reason
  * `never-run` — grey "No check has run yet" with a hint about the
    :17 hourly cron
- Refresh button (`admin-drift-status-refresh`) that re-fetches the
  latest row without waiting for the cron.
- Verified in preview: green state renders on a fresh check, and a
  seeded drift row produces the red state with the exploded testid
  list (`quarantine-empty-back-link`, `foo-testid-bar`, `another-missing-id`).

### Testids added

`admin-drift-status-card`, `admin-drift-status-badge`,
`admin-drift-status-label`, `admin-drift-status-meta`,
`admin-drift-status-error`, `admin-drift-status-refresh`,
`admin-drift-status-toggle`, `admin-drift-status-list`,
`admin-drift-item-<testid>`.

---

## 2026-07-09 — P0 Reconstruction: 17 missing testids restored, deploy unblocked

The exhaustive drift check (`scripts/deploy_drift_check.py`) had flagged 17
testids present in the currently-deployed prod bundle but MISSING from
source — deploying preview would have deleted the Quarantine flow +
Duplicate-dismissals card from production.  All 17 have been reconstructed
by reverse-engineering the cached prod bundle at `/tmp/prod_rb.js`.  Drift
check now reports **0 prod-only testids**.  Testing agent verified 14/14
backend cases + all 17 frontend testids render.

### Shipped

- **NEW `backend/routes/library_quarantine.py`** — 5 endpoints:
  - `GET  /api/library/quarantine` — groups `duplicate_pending` books by
    keeper, returns `{count, groups[{keeper, duplicates[{match_reasons,
    shared_fanfic_urls, has_cover, source_url, ...}]}]}`.
  - `POST /api/library/quarantine/{book_id}/resolve` with actions
    `keep_both` | `new_version` | `historical` | `discard` (30-day trash grace).
  - `POST /api/library/quarantine/{book_id}/not-duplicate` — writes to
    new `duplicate_dismissals` collection so future uploads matching
    the same (title, author, url) signature are silently un-flagged.
  - `GET  /api/user/duplicate-dismissals` — lists dismissals with keeper info.
  - `DELETE /api/user/duplicate-dismissals/{id}` — undo, re-arms dupe detection.
- **`backend/routes/books.py`** — `find_duplicate_candidates` now
  consults `_get_dismissed_keeper_ids` and filters keepers the user has
  already marked "not a duplicate" so bulk re-uploads don't re-flag.
- **NEW `frontend/src/pages/Quarantine.jsx`** — full page with empty state,
  grouped duplicates, per-row Keep/Promote/Historical/Discard actions,
  match-reason badges, and expandable "Why we caught this" panel.
- **NEW `frontend/src/components/DuplicateDismissalsCard.jsx`** — Account
  page card that only renders when user has >=1 dismissal; per-row Undo.
- **`frontend/src/components/Navbar.jsx`** — `navbar-quarantine` link
  added to the Library dropdown (both desktop and mobile drawer).
- **`frontend/src/App.js`** — `/library/quarantine` route wired.
- **`frontend/src/pages/Account.jsx`** — DuplicateDismissalsCard slotted
  above the Format-Prefs card.

### Restored testids (17)

`quarantine-summary`, `quarantine-empty-back-link`, `quarantine-group-*`,
`quarantine-discard-*`, `quarantine-keep-*`, `quarantine-promote-*`,
`quarantine-historical-*`, `quarantine-keeper-link-*`,
`quarantine-reason-*`, `navbar-quarantine`, `duplicate-dismissals-card`,
`duplicate-dismissals-count`, `dismissal-undo-*`, `dismissal-keeper-link-*`,
`not-duplicate-*`, `why-panel-*`, `why-toggle-*`.

### Deploy status

- `python3 scripts/deploy_drift_check.py` — **0 prod-only** (was 17).
- Backend + frontend supervisor RUNNING.
- Preview source is now safe to deploy.  All previously queued features
  (Wake Lock, orphan detection, presence rebuild, crossover URL persist,
  navbar-trash link) can ship together.
