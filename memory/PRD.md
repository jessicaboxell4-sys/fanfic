...
### Added 2026-06-14 (Book-club reading rooms — P2)
- Private invite-only spaces where a group of friends reads the same book together. Each room is tied to one book (owned by the creator) and serves as the canonical reference for the group.
- Backend: new `routes/bookclubs.py` (≈650 lines, 17 endpoints). Three new collections — `bookclubs`, `bookclub_members`, `bookclub_messages`.
  - Room CRUD: `POST /api/bookclubs` (creator must own the book; book's `chapters[]` populates `book_total_chapters`), `GET /api/bookclubs/{room_id}`, `PATCH /api/bookclubs/{room_id}` (owner/mod can edit name/desc/schedule), `DELETE /api/bookclubs/{room_id}` (owner only, cascade).
  - Listing: `GET /api/bookclubs` returns `{rooms: [active], invites: [pending]}` with `my_role`, `my_current_chapter` and `member_count` baked in via aggregation. `GET /api/bookclubs/pending-count` for navbar badges.
  - Membership: `POST /invite` (owner/mod, **friends-only gate** — uses existing `friendships` collection), `POST /accept`, `POST /decline`, `POST /leave` (owner can't leave with others present; lone-owner leave cascade-deletes the room), `POST /members/{uid}/role` (owner promote/demote), `POST /members/{uid}/remove` (owner/mod kick), `POST /transfer` (owner hands off, becomes moderator).
  - Discussion: `POST /messages` (body + `chapter_index`: 0=Lobby, N=chapter thread, validates ≤ `book_total_chapters`), `GET /messages?chapter=N` (chapter-filtered or full feed), fan-out notifications to other active members via existing `create_notification`.
  - Progress: `POST /progress` sets the user's `current_chapter`, clamps to total, fires `bookclub_finished` notification to other members when crossing the finish line.
- Frontend: two new pages.
  - **`BookclubsListPage.jsx`** (`/bookclubs`) — empty state, "New reading room" inline form (book picker from `/api/books`, name/schedule/description fields), pending-invites strip with Accept/Decline, active-room grid cards with role badge, schedule chip, progress bar (your chapter / total), and member count.
  - **`BookclubRoomPage.jsx`** (`/bookclubs/:roomId`) — sticky-tabs chapter selector (Lobby + 1..N) that syncs to `?chapter=N`; messaging panel with auto-scroll, 20s background polling, Ctrl/Cmd+Enter to send; right-rail with `MyProgressCard` (slider 0..total, optimistic update), `MembersCard` showing role icons (Crown = owner, ShieldCheck = moderator) and inline owner-only role/remove/transfer controls, and an `InviteFriendsBlock` (loads accepted friends, filters out current members). Owner-only Delete; member-only Leave.
- Wired discoverability: link from `/friends` header (`data-testid="friends-to-bookclubs"`) and a "Reading rooms" button in `/account` privacy section (`data-testid="privacy-open-bookclubs-btn"`). New routes registered in `App.js`.
